...
- **Scheduled single-engine run**: Cloud Scheduler passes `ENGINE_ID`, `ENGINE_LOCATION`, `ENDPOINT_LOCATION`, and `TABLE_ID_PREFIX` as environment overrides.
...
| `METRIC_FILTER` | No | Filter passed to `analytics:exportMetrics`. |
